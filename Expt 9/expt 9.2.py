...
plt.title('2D PCA of Iris Dataset By Sudheendra 211P048')
...
ax.set_title('3D PCA of Iris Dataset By Sudheendra 211P048')
# Manually create legend handles for each target class
handles_3d = []
for i in range(len(iris_target_names)):
  handles_3d.append(ax.scatter([], [], [], marker='o', s=100, edgecolor='k', c=[plt.cm.viridis(i / (len(iris_target_names) - 1))]))
ax.legend(handles_3d, iris_target_names, title="Species") # Use created handles 3D plot
plt.show()
